...
namespace EnchantElegance.Persistence.Implementations.Services
{
	internal class SliderService : ISliderService
	{
		...
		public async Task<PaginationVM<Slider>> GetAllAsync(int page = 1, int take = 3)
		{
			ICollection<Slider> sliders = await _sliderrepo.GetPagination(skip: (page - 1) * take, take: take).ToListAsync();

			int count = await _sliderrepo.GetAll().CountAsync();
